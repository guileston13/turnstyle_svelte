...
export const DEFAULT_CAMERA_ATTEMPTS = HIGH_RES_CAMERA_ATTEMPTS;
	...
	for (let pass = 0; pass <= retries; pass++) {
		for (const constraints of attempts) {
			try {
				...
				errors.push(new Error('Camera stream started without an active video track.'));
			} catch (error) {
				errors.push(error);
...
export async function requestCameraStream(
	...
	return startCameraStream({ attempts, retries: 0 });
}
...
export function getCameraFailure(error: unknown): CameraFailure {
	...
	return {
		kind: 'unknown',
		message: getCameraErrorMessage(error, 'Camera initialization failed.'),
